...
class db{
	...
	/*private $NumberQuery = false;*/ // Ресурс запрос
	...
	public function __construct($host, $user, $pass, $base){
		...
		$this->query("SET NAMES 'utf8'");		//кодировка
		$this->query("SET CHARACTER SET 'utf8'");		//кодировка
	}
	...
	Descriiption: Запрос
	\*======================================================================*/	
	public function query($query, /*$num_query = 0, */$FreeMemory = false, $write_last = true){
		
		$TimeA = $this->get_time();
		$xxt_res = mysqli_query($this->con, $query) or $this->GetError(mysqli_error($this->con));
		
		/*if ($num_query == 1) $this->NumberQuery = $xxt_res;
		else */if(/*!$num_query and */$write_last) $this->LastQuery = $xxt_res;
		...
		else */if (/*$num_query == 0 and */!$FreeMemory){
			return $this->LastQuery;
		}
	...
	public function query_recordless($query){
		$xxt_res = mysqli_query($this->con, $query) or $this->GetError(mysqli_error($this->con));
		return $xxt_res; 		//чтобы функция возвращала данные запроса
	}
	...
	/*function NumRows_NumberQuery()
	{
		return mysqli_fetch_assoc($this->NumberQuery);
	}*/
	...
	/*function FetchAssoc_NumberQuery(){		// создание массива конкретного запроса
	//if($this->LastQuery)
	return mysqli_fetch_assoc($this->NumberQuery);
	}*/
	/*======================================================================*\
