...
<?php
    if ($_SESSION['ADMIN_LOGIN_IN']) {
        ...
        echo '<br><a href="/page_p"><button type="submit" style="width: 500px; height: 30px; background-color: #929292; border: 1px #000 solid; color: #fff; cursor: pointer;">ПЕРЕЙТИ В СПИСОК ЗАПРОСОВ НА ВЫПЛАТЫ</button></a><br>';
        echo '<br><br>';

/*выводить все со статусом 0*/
        $db->Query("SELECT * FROM `history_money_payout` WHERE `id` != 1 AND (`bot` = 0 OR `bot` = 2) AND `status` = 0 AND `off` = 1"); //
        ...
        if ( !empty($numrows_h_m_p) ) {
            ...
            while( $as_h_m_p = $db->FetchAssoc() ) {
                ...
                <tr>
                        ...
                        <td><a href="/request?denied=1&id_list='.$as_h_m_p['id'].'&page_p_quest=1"><button type="submit" style="width: 80px; height: 30px; background-color: red; border: 1px #000 solid; color: #fff; cursor: pointer;">Отказ</button></a>
                        </form></td>
                        <td><a href="/request?trust=1&id_list='.$as_h_m_p['id'].'"><button type="submit" style="width: 80px; height: 30px; background-color: blue; border: 1px #000 solid; color: #fff; cursor: pointer;">Доверить</button></a>
                        </form></td>
                </tr>';
            }
            echo '
